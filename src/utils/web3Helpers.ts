...
export const shortenAddress = (address: string, chars = 4, isMobile = false): string => {
  const parsed = isAddress(address)
  if (!parsed) {
    throw Error(`Invalid 'address' parameter '${address}'.`)
  }

  return !isMobile ? `${parsed.substring(0, chars + 2)}...${parsed.substring(42 - chars)}` : `${parsed.substring(0, chars + 2)}`
}
